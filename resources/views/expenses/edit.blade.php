<x-app-layout>
    <x-slot name="header">Edit Expense &mdash; {{ $colocation->name }}</x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
            <form method="POST" action="{{ route('colocations.expenses.update', [$colocation, $expense]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="number" id="amount" name="amount" step="0.01" min="0.01" value="{{ old('amount', $expense->amount) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" id="date" name="date" value="{{ old('date', $expense->date->format('Y-m-d')) }}" required max="{{ now()->format('Y-m-d') }}"
                ...
                <div class="flex gap-3 mt-8">
                    ...
                    <a href="{{ route('colocations.expenses.show', [$colocation, $expense]) }}" class="px-6 py-2.5 border border-gray-300 hover:bg-gray-50 rounded-xl text-gray-700 font-medium transition">
                        Cancel
                    </a>
                </div>
            </form>
